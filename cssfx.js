...
var supported_rules = ["border-radius", "box-shadow", "text-overflow"];
...
for (var x = 0, xlen = styleElements.length; x < xlen; x++) {
	...
	var rules = [];
	var match_count = css.match(regex).length;
	for (var y = 0; y < match_count; y++) {
		...
		for (var z in supported_rules) {
			if (rule.indexOf(supported_rules[z]) != -1 && !has_supported_rules) {
